...
// Heartbeat: client posts position every couple of seconds. Returns the list
// of OTHER active players on the same server so the client can render ghost
// avatars of its teammates.
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    // Occasionally GC stale memberships system-wide. Only run on ~10% of calls
    // so every client isn't issuing a delete each heartbeat.
    if (Math.random() < 0.1) {
    ...
    if (identity) {
      // Make sure the player is allowed on this server before we insert presence.
